...
if __name__ == '__main__':
    ...
    box_mpc = Box(box_plant.right.add(3, -1.25), size=(5.5, 4.5), fill='{rgb,255:red,177;green,218;blue,236}',
                  draw=None, level=-1)
    ...
    Connection.connect(box_optimizer.input_bottom[1].sub_y(0.8), box_optimizer.input_bottom[1],
                       text=Text('Cost function', size=(2.5, 1)),
                       text_align='right', distance_x=1.1)

    # Open the document in the PDF Viewer and close it after 10s
    doc.open()
    time.sleep(10)
    doc.close()
