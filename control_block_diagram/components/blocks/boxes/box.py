...
class Box(Block):
    def __init__(self, position: (Point, Center), size: tuple = (2.5, 1.5), text: Text = None, fill: str = 'white',
                 draw: str = 'black', space: float = 1.5, in_out_space: float = None,
                 in_out_space_right_left: int = 0.2, in_out_space_top_bottom: int = 0.2, inputs: dict = dict(left=1),
                 outputs: dict = dict(right=1), doc=None):

        super().__init__(position, fill, draw, text, size, space, in_out_space_right_left, in_out_space_top_bottom, doc)
        ...
        input_dict = {'left': (
            ...
            self.right.add_y, 'east', self._size_y, inputs.get('right', 0), Input, inputs.get('right_text', ())),
                      'bottom': (
            self.bottom.add_x, 'south', self._size_x, inputs.get('bottom', 0), Input, inputs.get('bottom_text', ()))}
        ...
        output_dict = {'left': (
            ...
            self.right.add_y, 'east', self._size_y, outputs.get('right', 0), Output, outputs.get('right_text', ())),
                       'bottom': (
            self.bottom.add_x, 'south', self._size_x, outputs.get('bottom', 0), Output, outputs.get('bottom_text', ()))}
